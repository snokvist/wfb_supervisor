...
## Config shape
- `[general]`: `sse_tail`, `sse_host`, `sse_base_port`, and zero or more `init_cmd=` / `cleanup_cmd=` entries (run before starting instances and after shutdown). Commands run via `/bin/sh -c`, and these hooks are only valid in `[general]`.
